...
if [[ "$BOOTMODE" == "sdmmc" ]]; then
    ...
    SDMMC_IMAGE_FILE="$BINARIES_PATH/gsrd-console-image-agilex3.rootfs.wic"
...
SIMICS_TARGET="targets/agilex3c-universal/agilex3c-universal.simics"
...
if find modules -type f \( -name "Makefile" -o -name "*.py" \) | grep -q . && [[ -f "$SIMICS_TARGET" && -f "$SIMICS_BINARY" ]]; then
    ...
    simics_intelfpga_cli --deploy agilex3c-universal
    make
fi
