...
1. Copy required SOF files into the Yocto deploy directory:
   ```bash
   # Replace <sof_name> with your design sof file name (e.g., baseline)
   cp output_files/<sof_name>.sof yocto_linux/build/tmp/deploy/images/agilex3/
   ```

2. Copy scripts into the Yocto deploy directory:
   ```bash
   cp yocto_linux/scripts/* yocto_linux/build/tmp/deploy/images/agilex3/
   ```

3. Change to the Yocto deploy directory:
   ```bash
   cd yocto_linux/build/tmp/deploy/images/agilex3/
   ```
